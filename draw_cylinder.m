function [ output_args ] = untitled(pos, rot, dir, r, len, num_of_step, color)
...
% make circle surface
patch(topx' + pos(1), topy' + pos(2), topz' + pos(3), color, 'LineWidth', 1, 'LineStyle', '-');
patch(btmx' + pos(1), btmy' + pos(2), btmz' + pos(3), color, 'LineWidth', 1, 'LineStyle', '-');
...
siz = size(x2(1,:));
patch(x2(:,1    ) + pos(1), y2(:,1    ) + pos(2), z2(:,1    ) + pos(3), color, 'LineStyle', 'none');
patch(x2(:,2:end) + pos(1), y2(:,2:end) + pos(2), z2(:,2:end) + pos(3), color, 'LineStyle', 'none');
